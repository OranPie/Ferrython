...
_DECIMAL_DIGIT_RANGES = (
    ...
    0x11DA0, 0x11F50, 0x16A60, 0x16AC0, 0x16B50, 0x1D7CE, 0x1D7D8,
    0x1D7E2, 0x1D7EC, 0x1D7F6, 0x1E140, 0x1E2F0, 0x1E4F0, 0x1E950,
)

def _unicode_decimal_value(ch):
    code = ord(ch)
    for start in _DECIMAL_DIGIT_RANGES:
        if start <= code <= start + 9:
            return code - start
    try:
        return _unicodedata.decimal(ch)
    except Exception:
        return None
...
def _normalize_decimal_string(value):
    chars = []
    for ch in value:
        digit = _unicode_decimal_value(ch)
        chars.append(str(digit) if digit is not None else ch)
    return ''.join(chars)
...
class Decimal(object):
    ...
    def __setattr__(self, name, value):
        if name in ('real', 'imag', 'conjugate', '__complex__', '__decimal__'):
    ...
    def __new__(cls, value="0", context=None):
        ...
        if isinstance(value, str):
            ...
            cleaned = _normalize_decimal_string(cleaned)
            m = _parser(cleaned)
            if m is None:
                if context is None:
                    context = getcontext()
    ...
    def sqrt(self, context=None):
        ...
        try:
            ...
            orig_context._propagate_flags_from(context)
    ...
    def canonical(self, *args, **kwargs):
        ...
        """
        if args or kwargs:
            raise TypeError("canonical() takes no arguments")
        return self
    ...
    def copy_abs(self, *args, **kwargs):
        ...
        if args or kwargs:
            raise TypeError("copy_abs() takes no arguments")
        return _dec_from_triple(0, self._int, self._exp, self._is_special)

    def copy_negate(self, *args, **kwargs):
        """Returns a copy with the sign inverted."""
        if args or kwargs:
            raise TypeError("copy_negate() takes no arguments")
        if self._sign:
            return _dec_from_triple(0, self._int, self._exp, self._is_special)
        else:
...
def _signal_from_pickle_key(key):
    if key in _signals:
        return key
    name = getattr(key, '__name__', None)
    if name is None and isinstance(key, str):
        for signal in _signals:
            marker = 'Class(%s)' % signal.__name__
            if key == signal.__name__ or marker in key:
                return signal
    elif name is not None:
        for signal in _signals:
            if name == signal.__name__:
                return signal
    return key

def _normalize_signal_dict(d):
    return dict((_signal_from_pickle_key(k), v) for k, v in d.items())
...
class Context(object):
    ...
    def __init__(self, prec=None, rounding=None, Emin=None, Emax=None,
        ...
        if traps is None:
            ...
            self.traps = _normalize_signal_dict(traps)
        ...
        if flags is None:
            ...
            self.flags = _normalize_signal_dict(flags)
    ...
    def _propagate_flags_from(self, other):
        """Merge raised flags from another context into this one."""
        for signal, value in other.flags.items():
            if value:
                self.flags[signal] = value
...
def _convert_for_comparison(self, other, equality_op=False):
    ...
    # comparison result.
    if isinstance(other, _numbers.Rational) or (
            hasattr(other, 'numerator') and hasattr(other, 'denominator')):
        denominator = other.denominator
        numerator = other.numerator
        if not self._is_special:
            self = _dec_from_triple(self._sign,
                                    str(int(self._int) * denominator),
                                    self._exp)
        return self, Decimal(numerator)
