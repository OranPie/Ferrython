...
pub(super) fn py_format_value(obj: &PyObjectRef, spec: &str) -> PyResult<String> {
    ...
    let has_type_char = matches!(
        last_char,
        'd' | 'f' | 'F' | 'e' | 'E' | 'g' | 'G' | 'n' | 'b' | 'o' | 'x' | 'X'
    );
    ...
    match type_char {
        'd' => {
            ...
            let clean_spec: String = inner_spec
                .chars()
                .filter(|c| *c != ',' && *c != '_')
                .collect();
        ...
        'g' | 'G' => {
            ...
            let abs_f = f.abs();
            let use_exp = if abs_f == 0.0 {
                false
            } else {
                abs_f >= 10f64.powi(prec as i32) || abs_f < 1e-4
            };
            let result = if use_exp {
                ...
                let formatted = if abs_f == 0.0 {
                    ...
                    let decimal_places = if digits > mag {
                        (digits - mag) as usize
                    } else {
                        0
                    };
...
fn format_complex_with_spec(real: f64, imag: f64, spec: &str) -> PyResult<String> {
        ...
        let base = match type_char {
            ...
            'g' | 'G' => {
                ...
                let raw = if use_exp {
                    ...
                    let mag = if abs_f == 0.0 {
                        1
                    } else {
                        abs_f.log10().floor() as i32 + 1
                    };
                    let dp = if p as i32 > mag {
                        (p as i32 - mag) as usize
                    } else {
                        0
                    };
    ...
    if width > core.chars().count() {
        ...
        checked_repeat_len(1, pad, "format width")?;
