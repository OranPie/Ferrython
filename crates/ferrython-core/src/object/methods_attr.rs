...
pub(super) fn py_get_attr(obj: &PyObjectRef, name: &str) -> Option<PyObjectRef> {
    match &obj.payload {
            ...
            match name {
                ...
                _ => {
                    if name.starts_with("__") && name.ends_with("__") {
                        ...
                        static BUILTIN_DUNDERS: LazyLock<HashSet<&'static str>> =
                            LazyLock::new(|| {
                                [
                                    "__init__",
                                    "__new__",
                        ...
                        static CONTAINER_DUNDERS: LazyLock<HashSet<&'static str>> =
                            LazyLock::new(|| {
                                [
                                    "__len__",
                                    "__getitem__",
        ...
        PyObjectPayload::ExceptionType(kind) => {
            match name {
                "__name__" | "__qualname__" => Some(PyObject::str_val(CompactString::from(
                    format!("{:?}", kind),
                ))),
                "__bases__" => {
                    ...
                    let parent = match kind {
                        ...
                        | ExceptionKind::ConnectionRefusedError => {
                            Some(ExceptionKind::ConnectionError)
                        }
                    ...
                    if !found_cls {
                        if let PyObjectPayload::Class(ccd) = &cls.payload {
                            for base in &ccd.bases {
                                ...
                                if matches!(&base.payload, PyObjectPayload::ExceptionType(_)) {
                                    if let Some(resolved) =
                                        resolve_exception_type_method(name, instance)
                                    {
                                        return Some(PyObjectRef::new(PyObject {
                                            payload: PyObjectPayload::BoundMethod {
                                                receiver: instance.clone(),
