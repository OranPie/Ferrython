...
fn set_iterator_state(iter: &PyObjectRef, args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    if args.len() != 1 {
...
fn analyze_trivial_init(code: &CodeObject) -> Option<Vec<(usize, usize)>> {
    ...
    while i + 3 <= len {
            ...
            && instrs[i + 2].op == Opcode::StoreAttr
        {
            ...
            if arg_idx == 0 {
                return None;
            } // loading self as value — not a simple attr store
    ...
    if const_idx < code.constants.len() {
        if !matches!(
            ...
            ferrython_bytecode::code::ConstantValue::None
        ) {
            return None;
        }
    }
