            ...
            match instr.op {
                ...
                Opcode::StoreName => {
                    ...
                    match frame.scope_kind {
                        ...
                        ScopeKind::Class => {
                            frame.local_names_insert(name, value);
                        }
                        ScopeKind::Function => {
                            frame.local_names_insert(name, value);
                        }
                ...
                Opcode::LoadGlobal => {
                    ...
                    let cache = frame
                        .global_cache
                        .get_or_insert_with(|| Rc::new(vec![None; frame.code.names.len()]));
                        ...
                        match self.call_object(method, vec![name_arg]) {
                            ...
                            Err(e) if e.kind == ExceptionKind::AttributeError => {
                                ...
                                if let Some(ga2) = obj.get_attr("__getattr__") {
                                    let name_arg2 =
                                        PyObject::str_val(intern::intern_or_new(name.as_str()));
                ...
                if ferrython_core::object::is_property_like(&v) {
                    ...
                    if let Some(func) = func {
                        ...
                        if let PyObjectPayload::Instance(ref inst) = obj.payload {
                            inst.attrs
                                .write()
                                .insert(intern::intern_or_new(name.as_str()), result.clone());
                        }
                        self.vm_push(result);
                    } else {
                        self.vm_push(v);
                    ...
                    let get_method_bound =
                        if matches!(&get_method.payload, PyObjectPayload::BoundMethod { .. }) {
                            get_method
                        } else {
                    ...
                    let result =
                        self.call_object(get_method_bound, vec![instance_arg, owner_arg])?;
                    self.vm_push(result);
            ...
            if let Some(desc) = lookup_in_class_mro(&inst.class, name) {
                ...
                if is_data_descriptor(&desc) {
                    if let Some(set_method) = desc.get_attr("__set__") {
                        let set_bound =
                            if matches!(&set_method.payload, PyObjectPayload::BoundMethod { .. }) {
                                set_method
                            } else {
                                PyObjectRef::new(PyObject {
                                    payload: PyObjectPayload::BoundMethod {
        ...
        match &obj.payload {
            PyObjectPayload::Instance(inst) => {
                ...
                if let Some(class_attr) = lookup_in_class_mro(&inst.class, name.as_str()) {
                    ...
                    } else if let Some(delattr_method) =
                        lookup_in_class_mro(&inst.class, "__delattr__")
                    {
