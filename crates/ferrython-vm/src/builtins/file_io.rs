...
pub(super) fn builtin_open(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    let state: Arc<parking_lot::RwLock<FileState>> =
        Arc::new(parking_lot::RwLock::new(FileState::new(&path, &mode)?));
    ...
    all_attrs.insert(
        ...
        PyObject::native_function("read", file_read),
    );
    all_attrs.insert(
        CompactString::from("readline"),
        PyObject::native_function("readline", file_readline),
    );
    all_attrs.insert(
        CompactString::from("readlines"),
        PyObject::native_function("readlines", file_readlines),
    );
    all_attrs.insert(
        CompactString::from("write"),
        PyObject::native_function("write", file_write),
    );
    all_attrs.insert(
        CompactString::from("writelines"),
        PyObject::native_function("writelines", file_writelines),
    );
    all_attrs.insert(
        CompactString::from("close"),
        PyObject::native_function("close", file_close),
    );
    all_attrs.insert(
        CompactString::from("__enter__"),
        PyObject::native_function("__enter__", file_enter),
    );
    all_attrs.insert(
        CompactString::from("__exit__"),
        PyObject::native_function("__exit__", file_exit),
    );
    all_attrs.insert(
        CompactString::from("seek"),
        PyObject::native_function("seek", file_seek),
    );
    all_attrs.insert(
        CompactString::from("tell"),
        PyObject::native_function("tell", file_tell),
    );
    all_attrs.insert(
        CompactString::from("flush"),
        PyObject::native_function("flush", file_flush),
    );
    all_attrs.insert(
        CompactString::from("truncate"),
        PyObject::native_function("truncate", file_truncate),
    );
    all_attrs.insert(
        CompactString::from("readable"),
        PyObject::native_function("readable", file_readable),
    );
    all_attrs.insert(
        CompactString::from("writable"),
        PyObject::native_function("writable", file_writable),
    );
    all_attrs.insert(
        CompactString::from("seekable"),
        PyObject::native_function("seekable", file_seekable),
    );
    all_attrs.insert(
        CompactString::from("__iter__"),
        PyObject::native_function("__iter__", file_iter),
    );
    all_attrs.insert(
        CompactString::from("__next__"),
        PyObject::native_function("__next__", file_next),
    );
    all_attrs.insert(
        CompactString::from("isatty"),
        PyObject::native_function("isatty", file_isatty),
    );
    all_attrs.insert(
        CompactString::from("fileno"),
        PyObject::native_function("fileno", file_fileno),
    );
    all_attrs.insert(
    ...
    Ok(PyObject::module_with_attrs(
        CompactString::from("_file"),
        all_attrs,
    ))
}

static FILE_STATES: std::sync::LazyLock<
    Mutex<IndexMap<usize, Arc<parking_lot::RwLock<FileState>>>>,
...
fn get_file_state(args: &[PyObjectRef]) -> PyResult<Arc<parking_lot::RwLock<FileState>>> {
    let self_obj = args
        .first()
        .ok_or_else(|| PyException::type_error("file method called without self"))?;
    let ptr_val = self_obj
        .get_attr("_ptr")
        .ok_or_else(|| PyException::type_error("not a file object"))?;
    ...
    FILE_STATES
        ...
        .get(&ptr)
        .cloned()
        .ok_or_else(|| PyException::value_error("I/O operation on closed file"))
}
...
pub(super) fn file_write(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    if args.len() < 2 {
        return Err(PyException::type_error(
            "write() missing required argument: 'data'",
        ));
    }
    let state = get_file_state(args)?;
    let mut s = state.write();
...
pub(super) fn file_writelines(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    if args.len() < 2 {
        return Err(PyException::type_error(
            "writelines() missing required argument: 'lines'",
        ));
    }
    let state = get_file_state(args)?;
    let mut s = state.write();
...
pub(super) fn file_seek(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    if args.len() < 2 {
        return Err(PyException::type_error(
            "seek() missing required argument: 'offset'",
        ));
    }
    let state = get_file_state(args)?;
    let mut s = state.write();
...
pub(super) fn file_readable(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    Ok(PyObject::bool_val(
        s.mode.contains('r') || s.mode.contains('+'),
    ))
}
...
pub(super) fn file_writable(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    let s = state.read();
    Ok(PyObject::bool_val(
        s.mode.contains('w') || s.mode.contains('a') || s.mode.contains('+'),
    ))
}
