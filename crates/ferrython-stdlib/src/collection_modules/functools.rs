        ...
        vec![
                ...
                make_builtin(|args| {
                    ...
                    if let Some(last) = args.last() {
                        if let PyObjectPayload::Dict(d) = &last.payload {
                            let r = d.read();
                            if let Some(ms) =
                                r.get(&HashableKey::str_key(CompactString::from("maxsize")))
                            {
                                extracted_maxsize =
                                    Some(if matches!(ms.payload, PyObjectPayload::None) {
                                        None
                                    } else {
                ...
                make_builtin(|args| {
                    ...
                    if args.is_empty() {
                        return Err(PyException::type_error(
                            "cache requires a callable argument",
                        ));
                    }
            ...
            (
                ...
                make_builtin(|args| {
                    if args.is_empty() {
                        return Err(PyException::type_error(
                            "cached_property requires 1 argument",
                        ));
                    }
                    ...
                    attrs.insert(
                        CompactString::from("__cached_property_func__"),
                        func.clone(),
                    );
            ...
            (
                ...
                make_builtin(|args: &[PyObjectRef]| {
                    ...
                    if args.is_empty() {
                        return Err(PyException::type_error(
                            "partialmethod requires at least 1 argument",
                        ));
                    }
                    let func = args[0].clone();
                    let bound_args: Vec<PyObjectRef> = args[1..].to_vec();
                    let cls = PyObject::class(
                        CompactString::from("partialmethod"),
                        vec![],
                        IndexMap::new(),
                    );
                    ...
                    if let PyObjectPayload::Instance(ref d) = inst.payload {
                        ...
                        w.insert(
                            ...
                            PyObject::tuple(bound_args.clone()),
                        );
                        w.insert(
                        ...
                        w.insert(
                            ...
                            PyObject::native_closure("partialmethod.__get__", move |get_args| {
            ...
            ("_CacheInfo", {
                ...
                let cls =
                    PyObject::class(CompactString::from("_CacheInfo"), vec![], IndexMap::new());
                    ...
                    cd.namespace.write().insert(
                        ...
                        make_builtin(|args: &[PyObjectRef]| {
                            ...
                            if let PyObjectPayload::Instance(ref d) = inst.payload {
                                let mut w = d.attrs.write();
                                w.insert(
                                    CompactString::from("hits"),
                                    args.get(1).cloned().unwrap_or_else(|| PyObject::int(0)),
                                );
                                w.insert(
                                    CompactString::from("misses"),
                                ...
                                w.insert(
                                    CompactString::from("currsize"),
                                    args.get(4).cloned().unwrap_or_else(|| PyObject::int(0)),
                                );
                            }
                            Ok(PyObject::none())
                        }),
...
fn functools_total_ordering(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    if args.is_empty() {
        return Err(PyException::type_error(
            "total_ordering requires 1 argument",
        ));
    }
    let cls = &args[0];
    if let PyObjectPayload::Class(cd) = &cls.payload {
        let has = |name: &str| -> bool { cd.namespace.read().contains_key(name) };
        // Determine the root method
        let root = if has("__lt__") {
            "__lt__"
        } else if has("__gt__") {
            "__gt__"
        } else if has("__le__") {
        ...
        ns.insert(
            CompactString::from("__total_ordering_root__"),
...
fn functools_partial(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    let func = args[0].clone();
    let partial_args = if args.len() > 1 {
        args[1..].to_vec()
    } else {
        vec![]
    };
    Ok(PyObject::wrap(PyObjectPayload::Partial(Box::new(
        PartialData {
            func,
            args: partial_args,
            kwargs: vec![],
...
fn create_cached_function(func: PyObjectRef, maxsize: Option<i64>) -> PyObjectRef {
    let cache_class = PyObject::class(CompactString::from("_lru_wrapper"), vec![], IndexMap::new());
    ...
    let cache_info_fn = PyObject::native_closure("cache_info", move |_args| {
        ...
        let hits = r
            .get(&CompactString::from("_hits"))
            .and_then(|v| v.as_int())
            .unwrap_or(0);
        let misses = r
            .get(&CompactString::from("_misses"))
            .and_then(|v| v.as_int())
            .unwrap_or(0);
        ...
        if let PyObjectPayload::Instance(ref d) = info.payload {
            ...
            w.insert(
                ...
                PyObject::native_closure("CacheInfo.__repr__", move |_args| {
                    let ms_str = match ms {
                        Some(n) => n.to_string(),
                        None => "None".to_string(),
                    };
...
fn functools_singledispatch(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    if args.is_empty() {
        return Err(PyException::type_error(
            "singledispatch requires 1 argument",
        ));
    }
    ...
    cls_ns.insert(
        ...
        PyObject::native_function("singledispatch.register", |_args| {
            // Stub — actual dispatch handled by VM intercept
            Err(PyException::type_error(
                "singledispatch.register needs VM access",
            ))
        }),
    );
    ...
    cls_ns.insert(
        ...
        PyObject::native_function("singledispatch.__call__", |_args| {
            Err(PyException::type_error(
                "singledispatch.__call__ should be VM-intercepted",
            ))
        }),
    );
    ...
    if let PyObjectPayload::Instance(ref d) = inst.payload {
        let mut w = d.attrs.write();
        w.insert(
            CompactString::from("__singledispatch__"),
        ...
        if let PyObjectPayload::Dict(ref map) = registry.payload {
            map.write().insert(
                HashableKey::str_key(CompactString::from("object")),
                default_func.clone(),
            );
        }
