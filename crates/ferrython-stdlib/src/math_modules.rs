...
pub fn create_decimal_module() -> PyObjectRef {
    ...
    fn digits_compare(a: &[u8], b: &[u8]) -> std::cmp::Ordering {
        ...
        if a_len != b_len { return a_len.cmp(&b_len); }
    ...
    fn digits_subtract(a: &[u8], b: &[u8]) -> Vec<u8> {
        ...
        for i in (0..a.len()).rev() {
            ...
            let b_digit = if bi >= 0 && (bi as usize) < b.len() { b[bi as usize] as i8 } else { 0 };
        ...
        // Trim leading zeros
        let start = result.iter().position(|&d| d != 0).unwrap_or(result.len().saturating_sub(1));
        result[start..].to_vec()
    }
    ...
    fn digits_long_div(a: &[u8], b: &[u8]) -> Vec<u8> {
        ...
        let start = quotient.iter().position(|&d| d != 0).unwrap_or(quotient.len().saturating_sub(1));
    ...
    fn truncate_to_prec(s: &str, prec: u32) -> String {
        ...
        let round_digit = if keep < all_digits.len() { all_digits[keep].to_digit(10).unwrap_or(0) } else { 0 };
        let mut kept: Vec<u8> = all_digits[..keep].iter()
            .map(|c| c.to_digit(10).unwrap_or(0) as u8).collect();
        let round_up = if round_digit > 5 {
            ...
            else { kept.last().map_or(false, |&d| d % 2 != 0) }
        } else { false };
        if round_up {
            let mut i = kept.len();
            while i > 0 {
        ...
        if frac_part.is_empty() || keep <= int_len {
            ...
            let pad = if int_len > trunc_str.len() { int_len - trunc_str.len() } else { 0 };
            let padded = format!("{}{}", int_digits, "0".repeat(pad));
            if neg && padded != "0" { format!("-{}", padded) } else { padded }
        } else {
            let int_d = &trunc_str[..int_len];
            let frac_d = &trunc_str[int_len..];
            if neg { format!("-{}.{}", int_d, frac_d) } else { format!("{}.{}", int_d, frac_d) }
        }
    }
    ...
    fn decimal_div(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
        ...
        let formatted = if total_scale == 0 {
            if neg && result_str != "0" { format!("-{}", result_str) } else { result_str }
        } else {
