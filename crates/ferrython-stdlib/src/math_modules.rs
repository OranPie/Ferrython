...
fn bisect_left(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    let items = args[0].to_list()?;
    let lo = if args.len() > 2 {
        args[2].to_int()? as usize
    } else {
        0
    };
    let hi = if args.len() > 3 {
        args[3].to_int()? as usize
    } else {
        items.len()
    };
    Ok(PyObject::int(
        bisect_left_idx(&items, &args[1], lo, hi) as i64
    ))
}
...
fn bisect_right(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    Ok(PyObject::int(
        bisect_right_idx(&items, &args[1], lo, hi) as i64
    ))
}
...
fn insort_left(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    if let PyObjectPayload::List(lock) = &args[0].payload {
        ...
        Err(PyException::type_error(
            "insort_left: first arg must be a list",
        ))
    }
}
...
fn insort_right(args: &[PyObjectRef]) -> PyResult<PyObjectRef> {
    ...
    if let PyObjectPayload::List(lock) = &args[0].payload {
        ...
        Err(PyException::type_error(
            "insort_right: first arg must be a list",
        ))
    }
}
