...
use ferrython_core::object::{PyCell, PyObject, PyObjectMethods, PyObjectRef};
...
pub fn create_task_object(coroutine: &PyObjectRef) -> PyObjectRef {
    let task_cls = PyObject::class(CompactString::from("Task"), vec![], IndexMap::new());
    ...
    attrs.insert(
        CompactString::from("_state"),
        PyObject::str_val(CompactString::from("PENDING")),
    );
    ...
    attrs.insert(
        ...
        PyObject::native_closure("Task.done", move |_| Ok(PyObject::bool_val(h.is_done()))),
    );
    ...
    attrs.insert(
        ...
        PyObject::native_closure("Task.result", move |_| h.get_result()),
    );
    ...
    attrs.insert(
        ...
        PyObject::native_closure("Task.cancel", move |_| Ok(PyObject::bool_val(h.cancel()))),
    );

    // cancelled()
    let h = handle.clone();
    attrs.insert(
        CompactString::from("cancelled"),
        PyObject::native_closure("Task.cancelled", move |_| {
            Ok(PyObject::bool_val(*h.state.read() == TaskState::Cancelled))
        }),
    );
    ...
    let h = handle.clone();
    attrs.insert(
        CompactString::from("add_done_callback"),
        PyObject::native_closure("Task.add_done_callback", move |args| {
            if !args.is_empty() {
    ...
    let _h = handle.clone();
    attrs.insert(
        CompactString::from("remove_done_callback"),
        PyObject::native_closure("Task.remove_done_callback", move |_args| {
            Ok(PyObject::int(0))
        }),
    );
    ...
    attrs.insert(
        CompactString::from("get_name"),
        PyObject::native_closure("Task.get_name", move |_| {
            Ok(PyObject::str_val(nr.read().clone()))
        }),
    );
    let nr = name_ref.clone();
    attrs.insert(
        CompactString::from("set_name"),
        PyObject::native_closure("Task.set_name", move |args| {
            if !args.is_empty() {
    ...
    attrs.insert(
        ...
        PyObject::native_closure("Task.__repr__", move |_| {
    ...
    attrs.insert(
        CompactString::from("__await__"),
        PyObject::native_closure("Task.__await__", move |_| Ok(coro.clone())),
    );
...
pub fn create_future_object() -> PyObjectRef {
    ...
    attrs.insert(
        CompactString::from("done"),
        PyObject::native_closure("Future.done", move |_| {
            Ok(PyObject::bool_val(*s.read() != TaskState::Pending))
        }),
    );
    ...
    attrs.insert(
        CompactString::from("result"),
        PyObject::native_closure("Future.result", move |_| match *s.read() {
            TaskState::Pending => Err(PyException::new(
                ExceptionKind::RuntimeError,
                "Result is not ready",
            )),
    ...
    let r = result.clone();
    attrs.insert(
        CompactString::from("set_result"),
        PyObject::native_closure("Future.set_result", move |args| {
    ...
    let e = exception.clone();
    attrs.insert(
        CompactString::from("set_exception"),
        PyObject::native_closure("Future.set_exception", move |args| {
            if *s.read() != TaskState::Pending {
                return Err(PyException::new(
                    ExceptionKind::RuntimeError,
                    "Future already done",
    ...
    attrs.insert(
        CompactString::from("cancel"),
        PyObject::native_closure("Future.cancel", move |_| {
            let mut st = s.write();
    ...
    attrs.insert(
        CompactString::from("cancelled"),
        PyObject::native_closure("Future.cancelled", move |_| {
            Ok(PyObject::bool_val(*s.read() == TaskState::Cancelled))
        }),
    );
    ...
    attrs.insert(
        CompactString::from("__await__"),
        PyObject::native_closure("Future.__await__", move |_| {
            match *s.read() {
                TaskState::Finished => {
                    if let Some(exc) = e.read().clone() {
                        Err(exc)
