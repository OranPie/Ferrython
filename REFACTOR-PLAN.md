...
- Phase 2 system module mechanical splits have started. The low-coupling back
  ...
  `sysconfig`, `grp`, `pwd`, `os.path`, and sys stdio objects. The remaining
  top-level `sys_modules.rs` still owns `sys` and `os`, which are more tightly
  coupled to process state and path/file-descriptor helpers.
